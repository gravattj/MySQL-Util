...
sub _map_tables {
    args
        ...
        my $path => { isa => 'ArrayRef', optional => 1, default => [] };
    my ( $child_schema,  $child_table )  = $self->_un_fq($child);
    my ( $parent_schema, $parent_table ) = $self->_un_fq($parent);
    ...
    my $fks_href = $self->get_fk_constraints($child);

    foreach my $cons_name ( keys %$fks_href ) {

        my $cons_aref = $fks_href->{$cons_name};

        foreach my $col_href (@$cons_aref) {

            if (    $col_href->{REFERENCED_TABLE_NAME} eq $parent_table
                and $col_href->{REFERENCED_TABLE_SCHEMA} eq $parent_schema )
            {
                # found parent table
                return $path;
            }
    ...
    foreach my $cons_name ( keys %$fks_href ) {
        ...
        foreach my $col_href (@$cons_aref) {
            ...
            push( @{$path}, $parent_table );

            my $found_path = $self->_map_tables(
                child  => $parent,
                parent => $self->_fq(
                    table  => $col_href->{REFERENCED_TABLE_NAME},
                    schema => $col_href->{REFERENCED_TABLE_SCHEMA},
                    fq     => 1
                ),
                path => $path
            );
...
sub _get_fk_sql {
    ...
    my $con_aref = $self->get_constraint( table => $table, name => $fk_name );
...
sub _get_ak_data {
    ...
    if ( $self->has_ak($table) ) {
        ...
        $self->_get_uniq_constraint_data(
            table           => $table,
            col_data_href   => $col_data_href,
            constraint_name => $self->get_ak_name($table)
        );
    }
}
