...
sub get_ak_constraint {
    ...
    eval { $Util->get_ak_constraint };
    ok( $@, "$func - called without args" );

    eval { $Util->get_ak_constraint('garbage') };
    ok( $@, "$func - called with garbage" );

    my $ak;
    ok( $ak = $Util->get_ak_constraint('table_b'),
        "$func - called with valid table"
    );

    ok( ref $ak eq 'HASH', "$func - result is a hashref" );
    ok( scalar keys(%$ak), "$func - result has 1 key" );

    my ($constraint_name) = keys %$ak;
    ok( $constraint_name eq 'table_b_ak', "$func - constraint name" );

    my $aref = $ak->{$constraint_name};
    ok( ref $aref eq 'ARRAY', "$func - columns are an arrayref" );
...
sub table_exists {
    ...
    $table = 'garbage';
    ok( !$Util->table_exists($table),
        "$func - table $table does not exist " );
